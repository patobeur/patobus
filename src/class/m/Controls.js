...
class Controls {
	...
	_setupDeviceControls() {
		if (
			!this.detectDevice.isMousePointer &&
			(this.detectDevice.touchEvent ||
				this.detectDevice.ontouchstart ||
				this.detectDevice.maxTouchPoints)
		) {
		...
		if (
			this.detectDevice.isMousePointer &&
			this.detectDevice.maxTouchPoints === false
		) {
		...
		if (this.detectDevice.isMousePointer && this.detectDevice.maxTouchPoints) {
			this._touchDeviceActive = false;
			console.log(
				"------------> Keyboard'n'Pad on, Sorry you need to conect a Mouse and refresh [5] ! ⌨️"
			);
			this._addKeyboardListeners();
			this._addMouseListeners();
		}
	...
	_handleMouseMove(event, target) {
		...
		this.thetaDeg = this.formula.get_DegreeWithTwoPos(
			window.innerWidth / 2,
